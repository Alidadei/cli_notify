...
通知 hooks 以 **Claude Code 插件** 形式独立部署，与 settings 中的 permissions 物理隔离：

```
~/.claude/plugins/notify/
├── .claude-plugin/plugin.json    # 插件元信息
└── hooks/hooks.json              # Stop + PreToolUse hooks
```

- hooks 不属于 LLM 上下文，在 harness 层面注册，不占用 context window
- 修改 settings 的 permissions 不会影响通知功能
- 一键安装脚本 (`notify-setup.ps1`) 会自动创建插件文件，并清理 settings 中残留的旧 hooks

### Windows Terminal 兼容

进程树遍历查找窗口时不再限制 `cmd.exe`，而是接受任意有可见窗口的祖先进程：

```
wt.exe (窗口) → cmd.exe → claude.exe → powershell (hook) → notify.ps1
                                  ↑ 旧逻辑在这里停住（cmd.exe 在 wt 中无独立窗口）
                    ↑ 新逻辑继续向上，找到 wt.exe 的 MainWindowHandle
```

### 通知流程

```
Claude Code 事件 (Stop / PreToolUse)
  → hooks.json 匹配
    → 执行 notify.ps1 (stdin 传入 JSON)
      → 解析 payload、去重检查
        → Windows Toast (BurntToast / 原生)
        → 企业微信 Webhook (可选)
        → Telegram Bot (可选)
```
